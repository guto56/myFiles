...
# Lista inicial de códigos de licença válidos
codigos_licenca = [
...
# Função para salvar os códigos disponíveis em um arquivo
def salvar_codigos():
    with open("codigos_disponiveis.txt", "w") as f:
        for codigo in codigos_licenca:
            f.write(codigo + "\n")

# Função para carregar os códigos disponíveis do arquivo
def carregar_codigos():
    global codigos_licenca
    try:
        with open("codigos_disponiveis.txt", "r") as f:
            codigos_licenca = [linha.strip() for linha in f.readlines()]
    except FileNotFoundError:
        salvar_codigos()  # Se o arquivo não existir, salva os códigos originais

# Função para gerar um código de licença aleatório
def gerar_codigo():
    if not codigos_licenca:
        codigo_gerado.set("Não há códigos disponíveis.")
        return

    codigo = random.choice(codigos_licenca)
    codigos_licenca.remove(codigo)  # Remover o código gerado da lista
    salvar_codigos()  # Atualiza o arquivo de códigos disponíveis

    codigo_gerado.set(codigo)  # Exibir o código gerado na caixa de texto

# Criar a interface gráfica
def criar_interface():
    global codigo_gerado
    
    # Carregar os códigos disponíveis
    carregar_codigos()

    janela = tk.Tk()
    janela.title("Gerador de Licença")
    janela.geometry("400x200")  # Ajuste o tamanho da janela

    fonte_padrao = tkfont.Font(family="Segoe UI", size=12)

    # Texto explicativo
    label = tk.Label(janela, text="Clique em 'Gerar' para criar um código de licença:", font=fonte_padrao)
    label.pack(pady=10)

    # Caixa de entrada de texto para exibir o código gerado
    codigo_gerado = tk.StringVar()
    campo_codigo = tk.Entry(janela, font=fonte_padrao, width=40, textvariable=codigo_gerado, state="readonly")
    campo_codigo.pack(pady=10)

    # Botão para gerar o código
    botao_gerar = tk.Button(janela, text="Gerar", command=gerar_codigo, font=fonte_padrao, bg="lightgreen", relief="raised")
